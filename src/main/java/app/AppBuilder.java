...
public class AppBuilder {
    ...
    public AppBuilder addTestDisplayPlayersUseCase() {
        ...
        // 2. View
        testScrollableListViewV2 = new TestScrollableListViewV2(testDisplayPlayersViewModel, viewManagerModel);
        cardPanel.add(testScrollableListViewV2, testScrollableListViewV2.getViewName());

        // 3. Presenter
        TestDisplayPlayersPresenter presenter = new TestDisplayPlayersPresenter(testDisplayPlayersViewModel);

        // 4. Interactor
        TestDisplayPlayersInteractor interactor = new TestDisplayPlayersInteractor(playerDataAccess,  // Already exists
                presenter);

        // 5. Controller
        testDisplayPlayersController = new TestDisplayPlayersController(interactor);
    ...
    public AppBuilder addInitialisePredictions() {
        ...
        InitialisePredictionsPresenter presenter = new InitialisePredictionsPresenter(initViewModel, viewManagerModel, "home"  // Name of the home view to switch to when done
        );
        ...
        ModelCoefficientDataAccessInterface coefficientAccess = new ModelCoefficientDataGateway();

        // Create Interactor
        InitialisePredictionsInputBoundary interactor = new InitialisePredictionsInteractor(bootstrapAccess, gameweekAccess, coefficientAccess, presenter, playerDataAccess);
    ...
    public AppBuilder addRiskAssessmentUseCase() {
        ...
        RiskAssessmentPresenter presenter = new RiskAssessmentPresenter(riskAssessmentViewModel, viewManagerModel);

        // Interactor depends on TeamDataAccessInterface
        RiskAssessmentInteractor interactor = new RiskAssessmentInteractor(teamAccess, playerDataAccess, presenter);
    ...
    public AppBuilder addHomeUseCase() {
        final HomeController homeController = new HomeController(homeViewModel, teamEntryController, teamEntryInputBoundary, startingLineupController, displayIndividualStatController, riskAssessmentController, transferSuggestionsController, viewManagerModel);
    ...
    public AppBuilder addStartingLineupView() {
        TeamViewModel.DisplayConfig lineupConfig = new TeamViewModel.DisplayConfig("Starting Lineup", "No valid lineup available.", false, new String[]{"Name", "Position", "Club", "Price", "Points"});
    ...
    public AppBuilder addTeamEntryViewUseCase() {
        final TeamEntryPresenter presenter = new TeamEntryPresenter(viewManagerModel, teamEntryViewModel, homeViewModel);

        final TeamEntryInteractor interactor = new TeamEntryInteractor(presenter, teamDataAccess);
    ...
    public AppBuilder addDisplayIndividualStatUseCase() {
        final DisplayIndividualStatOutputBoundary outputBoundary = new DisplayIndividualStatPresenter(viewManagerModel, displayIndividualStatViewModel);

        final DisplayIndividualStatInputBoundary interactor = new DisplayIndividualStatInteractor(outputBoundary, playerDataAccess);

        displayIndividualStatController = new DisplayIndividualStatController(interactor);
    ...
    public AppBuilder addTransferSuggestionsView() {
        ...
        transferSuggestionsView = new TransferSuggestionsView(transferSuggestionsViewModel, viewManagerModel);
    ...
    public AppBuilder addTransferSuggestionsUseCase() {
        // Create Presenter
        final TransferSuggestionsPresenter presenter = new TransferSuggestionsPresenter(transferSuggestionsViewModel, viewManagerModel, homeViewModel);

        // Create Interactor
        final TransferSuggestionsInteractor interactor = new TransferSuggestionsInteractor(transferSuggestionsTeamDataAccess,      // Need to add this field - see below
                playerDataAccess,    // Already exists
                presenter);

        // Create Controller
        transferSuggestionsController = new TransferSuggestionsController(interactor, transferSuggestionsViewModel);
